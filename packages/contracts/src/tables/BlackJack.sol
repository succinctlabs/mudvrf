...
library BlackJack {
    ...
    function getItemUserCards(address userAddress, uint256 _index) internal view returns (uint256) {
        ...
        bytes memory _blob =
            StoreSwitch.getFieldSlice(_tableId, _keyTuple, 4, getSchema(), _index * 32, (_index + 1) * 32);
        return (uint256(Bytes.slice32(_blob, 0)));
    }
    ...
    function getItemDealerCards(address userAddress, uint256 _index) internal view returns (uint256) {
        ...
        bytes memory _blob =
            StoreSwitch.getFieldSlice(_tableId, _keyTuple, 5, getSchema(), _index * 32, (_index + 1) * 32);
        return (uint256(Bytes.slice32(_blob, 0)));
    }
